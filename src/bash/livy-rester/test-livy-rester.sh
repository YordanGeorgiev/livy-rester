#!/bin/bash 
# file: src/bash/livy-rester/test-livy-rester.sh
...
doRunTests(){
   ...
   $product_instance_dir/src/bash/livy-rester/tests/run-livy-rester-tests.lst"
   doLog "INFO running the following actions : "
   cat $product_instance_dir/src/bash/livy-rester/tests/run-livy-rester-tests.lst | grep -v '#'
   sleep 1
      ...
      doLogTestRunEntry 'START'
		while read -r test_file ; do (
            ...
            then
               ...
               $function_name
               doLog "INFO test-livy-rester loop exit_code: $exit_code" 
               doLogTestRunEntry 'INFO' ' '$(date "+%H:%M:%S")
		...
		done < <(find src/bash/livy-rester/tests -type f -name '*.sh')

	);
	done < <(cat $product_instance_dir/src/bash/livy-rester/tests/run-livy-rester-tests.lst)
...
doLog(){
   ...
   test -t 1 && echo " [$type_of_msg] `date "+%Y.%m.%d-%H:%M:%S"` [livy-rester][@$host_name] [$$] $msg "
...
doSetVars(){
   ...
   . "$product_instance_dir/src/bash/livy-rester/funcs/log-test-run-entry.func.sh"
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the livy-rester
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: src/bash/livy-rester/test-livy-rester.sh
