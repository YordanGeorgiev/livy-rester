# src/bash/livy-rester/funcs/clone-project.func.sh

# v1.0.9
# ---------------------------------------------------------
# cat doc/txt/livy-rester/funcs/clone-project.func.txt
# ---------------------------------------------------------
doCloneProject(){
   ...
   # doc/txt/livy-rester/spec/clone-project.spec.txt
   export src_proj='ysg-issues'
   export tgt_proj='phz'
   export global_cnf_dir='/vagrant/csitea/cnf/projects/livy-rester'
   ...
   bash src/bash/livy-rester/livy-rester.sh -a morph-dir

   # start working again on the project to be cloned
   doParseIniEnvVars "$global_cnf_dir"'/'"$tgt_proj"".prd.host-name.cnf"

   # and increase the dat
   bash src/bash/livy-rester/livy-rester.sh -a increase-date -d today
...
# eof file: src/bash/livy-rester/funcs/clone-project.func.sh
