...
doGenerateActionFiles(){
		...
		doLog "DEBUG START :: checking action: $act"
			...
			while read -d" " deliverable_type ; do (
				...
				if [ $doc_file_exists -eq 0 ];then
					...
					cp -v doc/txt/livy-rester/tmpl/%act%.%deliverable_type%.txt "$deliverable_doc_file"
	...
	done< <(cat src/bash/$run_unit/tests/new-livy-rester-tests.lst)
   for env in `echo dev tst prd src`; do cp -v met/.$env_type.$run_unit met/.$env.$run_unit ; done
